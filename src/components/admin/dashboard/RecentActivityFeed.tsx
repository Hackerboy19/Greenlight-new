...
const ACTIONS: Record<ActivityAction, { verb: string; icon: React.ComponentType<{ className?: string }>; tone: string }> = {
  ...
  published: { verb: 'published', icon: CheckCircle2, tone: 'bg-brand-100 dark:bg-brand-950/60 text-brand-700 dark:text-brand-400' },
...
export const RecentActivityFeed: React.FC<RecentActivityFeedProps> = ({ entries, loading, onOpenArticle, canOpenArticle }) => (
  <Card as="section" className="flex flex-col">
    <SectionHeading level="h3" className="mb-4" title="Recent activity" subtitle="Who edited or published what" />
                ...
                <p className="text-slate-700 dark:text-slate-300 leading-snug">
                  ...
                  {canOpen ? (
                    <button
                      ...
                      className="font-semibold text-brand-700 dark:text-brand-400 hover:underline text-left"
                    >
